...
/*   Updated: 2019/10/12 13:08:33 by amatthys    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
{
	hash_init(cmd, hash);
	hash->registers[REG_A].x64 = 0x6a09e667f3bcc908;
	hash->registers[REG_B].x64 = 0xbb67ae8584caa73b;
	hash->registers[REG_C].x64 = 0x3c6ef372fe94f82b;
	hash->registers[REG_D].x64 = 0xa54ff53a5f1d36f1;
	hash->registers[REG_E].x64 = 0x510e527fade682d1;
	hash->registers[REG_F].x64 = 0x9b05688c2b3e6c1f;
	hash->registers[REG_G].x64 = 0x1f83d9abfb41bd6b;
	hash->registers[REG_H].x64 = 0x5be0cd19137e2179;
}

static void		ft_sha512_update_inner(t_alltypes *reg, size_t *block,
		unsigned i)
{
	size_t	tmp[2];
...
int				ft_sha512_update(t_hash_use *hash)
{
	size_t		block[80];
	t_alltypes	reg[8];
	unsigned	i;
	...
	i = -1;
	while (++i < 80)
		ft_sha512_update_inner(reg, block, i);
