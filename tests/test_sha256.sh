#/!bin/bash
SHA256_PERSO="./ft_ssl sha256 "
SHA256_ORIGIN="shasum -a 256 | cut -d' ' -f 1 "
...
test_sha256 ()
{
	...
	if [ -z $3 ]
	then
		(eval ${SHA256_PERSO} $2) > my_hash.output
		(eval ${SHA256_ORIGIN} $2) > origin_hash.output
	else
		(eval $3 "$4" | eval ${SHA256_PERSO} $2) > my_hash.output
		(eval $3 "$4" | eval ${SHA256_ORIGIN} $2) > origin_hash.output 
	fi
